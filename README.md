...
'Relational Databases' are data that are stored in a table format with rows and columns, while 'SQL' is the programming language used to manage the database itself.

2. Why do tables need a `primary key`?

Primary keys are necesarry to identify records easily in a database and make each on its records unique.
...
We need a junction table, which an intermediary table that created one-to-many relationships with two different tables.
